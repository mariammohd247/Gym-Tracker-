...
import { MACHINE_LABELS } from '@/lib/cardioCalories'
import {
...
// ── Cardio card ──────────────────────────────────────────────────
// Uses local state so typing never triggers parent re-renders mid-keystroke.
// Calories calculated via AI edge function on blur (no instant client-side calc).
function CardioCard({ ex, idx, onChange, onRemove, onCalcCalories }: {
  ...
  const [local, setLocal] = useState<CardioDraft>(ex)
  const [showMachineMenu, setShowMachineMenu] = useState(false)

  // Sync inbound changes (e.g. machine switch) but don't clobber local typing
  useEffect(() => { setLocal(ex) }, [ex.machine_type, ex.estimated_calories, ex.calculating]) // eslint-disable-line react-hooks/exhaustive-deps

  const isRowSki    = local.machine_type === 'row' || local.machine_type === 'ski'
  const isTreadmill = local.machine_type === 'treadmill'
  const isStairs    = local.machine_type === 'stairs'

  function patch(p: Partial<CardioDraft>) {
    setLocal(prev => ({ ...prev, ...p }))
  }

  function commit(p?: Partial<CardioDraft>) {
    const next = { ...local, ...(p ?? {}) }
    onChange(idx, next)
  }

  const { label, emoji } = MACHINE_LABELS[local.machine_type]
    ...
    <div className="bg-gray-800 border border-purple-700/40 rounded-2xl p-4 space-y-3">
              ...
              {(Object.entries(MACHINE_LABELS) as [MachineType, { label: string; emoji: string }][]).map(([key, val]) => (
                <button key={key} type="button"
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition ${
                    local.machine_type === key ? 'bg-purple-500/20 text-purple-300' : 'text-gray-200 hover:bg-gray-700'
                  }`}
                >
        ...
        <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex rounded-xl overflow-hidden border border-gray-600">
              {(['meters', 'calories'] as const).map(u => (
                <button key={u} type="button"
                  onClick={() => { patch({ cardio_unit: u }); commit({ cardio_unit: u }) }}
                  className={`flex-1 py-2 text-xs font-semibold transition ${
                    local.cardio_unit === u ? 'bg-purple-500 text-white' : 'bg-gray-700 text-gray-400 hover:text-white'
                  }`}
                >
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1.5 block">
              {local.cardio_unit === 'meters' ? 'Distance (m)' : 'Target Cal'}
            </label>
            <input type="number" value={local.cardio_target || ''} min={0}
              placeholder={local.cardio_unit === 'meters' ? '2000' : '100'}
              onChange={e => patch({ cardio_target: parseInt(e.target.value) || 0 })}
              onBlur={() => { commit(); onCalcCalories(idx) }}
          ...
          <div className="col-span-2">
            ...
            <div className="flex items-center gap-3">
              <input type="range" min={1} max={10} value={local.machine_level}
                onChange={e => patch({ machine_level: parseInt(e.target.value) })}
                onMouseUp={() => { commit(); onCalcCalories(idx) }}
                onTouchEnd={() => { commit(); onCalcCalories(idx) }}
                className="flex-1 accent-purple-500"
              />
              <span className="text-white font-bold w-6 text-center">{local.machine_level}</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="number" value={local.duration_minutes} min={1} max={300}
                onChange={e => patch({ duration_minutes: parseInt(e.target.value) || 1 })}
                onBlur={() => { commit(); onCalcCalories(idx) }}
                className="w-full bg-gray-700 border border-gray-600 rounded-xl px-3 py-2 text-white text-sm text-center focus:outline-none focus:border-purple-500 transition"
              />
            </div>
            <div>
              ...
              <input type="number" value={local.speed} min={0} step={0.5}
                onChange={e => patch({ speed: parseFloat(e.target.value) || 0 })}
                onBlur={() => { commit(); onCalcCalories(idx) }}
                className="w-full bg-gray-700 border border-gray-600 rounded-xl px-3 py-2 text-white text-sm text-center focus:outline-none focus:border-purple-500 transition"
              />
            </div>
          ...
          {isTreadmill && (
            <div className="grid grid-cols-2 gap-2">
                ...
                <div className="flex rounded-xl overflow-hidden border border-gray-600">
                  {(['run', 'walk'] as const).map(m => (
                    <button key={m} type="button"
                      onClick={() => { patch({ machine_mode: m }); commit({ machine_mode: m }); onCalcCalories(idx) }}
                      className={`flex-1 py-2 text-xs font-semibold transition capitalize ${
                        local.machine_mode === m ? 'bg-purple-500 text-white' : 'bg-gray-700 text-gray-400 hover:text-white'
                      }`}
                    >
                      {m === 'run' ? '🏃 Run' : '🚶 Walk'}
                    </button>
              ...
              <div>
                ...
                <input type="number" value={local.incline} min={0} max={30} step={0.5}
                  onChange={e => patch({ incline: parseFloat(e.target.value) || 0 })}
                  onBlur={() => { commit(); onCalcCalories(idx) }}
                  className="w-full bg-gray-700 border border-gray-600 rounded-xl px-3 py-2 text-white text-sm text-center focus:outline-none focus:border-purple-500 transition"
                />
              </div>
          ...
          {!isTreadmill && (
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="range" min={1} max={20} value={local.machine_level}
                  onChange={e => patch({ machine_level: parseInt(e.target.value) })}
                  onMouseUp={() => { commit(); onCalcCalories(idx) }}
                  onTouchEnd={() => { commit(); onCalcCalories(idx) }}
                  className="flex-1 accent-purple-500"
      ...
      <div className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm ${
        ...
        {ex.calculating ? (
          ...
          <span className="text-gray-500 text-xs">Fill in details above — AI will estimate calories</span>
        )}
      </div>
    </div>
  )
...
// ── Strength card ────────────────────────────────────────────────
// Uses local state for name so typing spaces never loses focus.
// Syncs to parent on blur. Calories via AI edge function only.
function StrengthCard({ ex, idx, onChange, onRemove, onCalcCalories }: {
  ...
  // Local name state — prevents parent re-renders from breaking typing
  const [localName, setLocalName] = useState(ex.name)

  // Keep in sync if parent resets (e.g. after load)
  useEffect(() => { setLocalName(ex.name) }, [ex.name]) // eslint-disable-line react-hooks/exhaustive-deps

  function commitName() {
    onChange(idx, 'name', localName)
    if (localName.trim()) onCalcCalories(idx)
  }
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-2xl p-4 space-y-3">
      ...
      {/* Reps / Sets / Weight — no duration shown */}
      <div className="grid grid-cols-3 gap-2">
        <div>
          ...
          <input type="number" value={ex.reps} min={1} max={100}
            ...
            onBlur={() => { if (ex.name.trim() || localName.trim()) onCalcCalories(idx) }}
        ...
        <div>
          <label className="flex items-center gap-1 text-xs text-gray-400 mb-1.5">
            <RotateCcw className="w-3 h-3" /> Sets
          </label>
      ...
      <div className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm ${
        ...
        {ex.calculating ? (
          ...
          <span className="text-gray-500 text-xs">Fill in details — AI will estimate calories on blur</span>
        )}
      </div>
    </div>
  )
...
export default function PreWorkoutAdmin() {
  ...
  // ── AI calorie calculation (OpenRouter via edge fn) ───────────
  async function calcStrengthCalories(idx: number) {
    ...
    try {
      const { data } = await supabase.functions.invoke('calculate-calories', {
        body: { type: 'strength', exerciseName: name, weightKg: ex.weight_kg, reps: ex.reps, rounds: ex.rounds, userWeightKg: TEMPLATE_WEIGHT_KG },
      })
  ...
  async function calcCardioCaloriesAI(idx: number) {
    ...
    try {
      const { data } = await supabase.functions.invoke('calculate-calories', {
        body: {
          type: 'cardio', machineType: ex.machine_type, durationMinutes: ex.duration_minutes,
          speed: ex.speed, meters: ex.cardio_unit === 'meters' ? ex.cardio_target : null,
          incline: ex.incline, machineMode: ex.machine_mode, machineLevel: ex.machine_level,
          cardioUnit: ex.cardio_unit, cardioTarget: ex.cardio_target, userWeightKg: TEMPLATE_WEIGHT_KG,
        },
      })
      setExercises(prev => prev.map((e, i) => i === idx
    ...
    const rows = exercises.map((ex, i) => {
      if (ex.exercise_type === 'cardio') {
        ...
        return {
          ...
          name: MACHINE_LABELS[c.machine_type]?.label ?? c.machine_type,
          exercise_type: 'cardio',
          base_calories: c.estimated_calories,
          estimated_calories: c.estimated_calories,
          duration_minutes: c.duration_minutes ?? 20,
          machine_type: c.machine_type, speed: c.speed, incline: c.incline,
          machine_mode: c.machine_mode, machine_level: c.machine_level,
          cardio_unit: c.cardio_unit, cardio_target: c.cardio_target,
          reps: 0, rounds: 0, weight_kg: 0, order_index: i,
        }
      }
      ...
      return {
        ...
        machine_type: null, speed: null, incline: null,
        machine_mode: null, machine_level: null, cardio_unit: null, cardio_target: null,
        order_index: i,
      }
    })
    if (rows.length > 0) await supabase.from('exercises').insert(rows)
  ...
  return (
    <div className="pb-28 space-y-5 px-4 pt-4">
        ...
        {showNewTypeForm && (
          <div className="border-t border-gray-700 p-4 space-y-3">
            ...
            <button onClick={addWorkoutType} disabled={savingType || !newTypeName.trim()}
              className="w-full bg-gradient-to-r from-orange-600 to-orange-400 disabled:opacity-40 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition active:scale-95"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Workout Types ({workoutTypes.length})</p>
                          ...
                          {exercises.map((ex, idx) =>
                            ex.exercise_type === 'cardio' ? (
                              <CardioCard key={idx} ex={ex as CardioDraft} idx={idx}
                                onChange={updateCardio} onRemove={removeExercise} onCalcCalories={calcCardioCaloriesAI}
                              />
                            ) : (
                              <StrengthCard key={idx} ex={ex as StrengthDraft} idx={idx}
                                onChange={updateStrength} onRemove={removeExercise} onCalcCalories={calcStrengthCalories}
                              />
                            )
                          )}
                        </div>
